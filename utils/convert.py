...
DIR = os.path.abspath(os.path.join(script_dir,'..', 'dspyt',  'public', 'images', 'posts', 'cybersecurity',))
#DIR = os.path.join('..', 'dspyt', 'public', 'authors')
#DIR = os.path.join('..', 'dspyt', 'public')

files = os.listdir(DIR)

images = [file for file in files if file.endswith(('jpg', 'png', 'jpeg', 'webp'))]

existing_webp_files = [file for file in images if file.lower().endswith('.webp')]
...
for element in images:
    ...
    if file_path.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        if os.path.isfile(webp_file_path):
            print(f"{element} already has a corresponding WebP file. Skipping conversion.")
        ...
        # can further add conversion
            ...
            if new_size < initial_size:
                ...
                print(f"{element} converted and size reduced: {(initial_size - new_size) / (1024 * 1024):.2f} MB")
                reduction_percent = ((initial_size - new_size) / initial_size) * 100
                print(f"{element} % size reduction: {reduction_percent:.2f} %")
            else:
                os.remove(webp_file_path)
                print(f"{element} converted but size not reduced, original file kept")

    elif file_path.lower().endswith('.webp'):
        if element in existing_webp_files:
            initial_size = os.path.getsize(file_path)
